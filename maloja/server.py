...
# Refer to: https://bottlepy.org/docs/dev/faq.html#problems-with-reverse-proxies
def fix_environ_middleware(app):
    def fixed_app(environ, start_response):
        environ["HTTP_X_FORWARDED_PROTO"] = os.environ.get("MALOJA_URL_SCHEME")
        environ["HTTP_X_FORWARDED_HOST"] = os.environ.get("MALOJA_URL_HOST")
        return app(environ, start_response)

    return fixed_app


webserver.wsgi = fix_environ_middleware(webserver.wsgi)
...
def jinja_page(name):
    if name in aliases:
        redirect(aliases[name])
    keys = remove_identical(FormsDict.decode(request.query))

    adminmode = request.cookies.get("adminmode") == "true" and auth.check_request(
        request
    )
    ...
    with JinjaDBConnection() as conn:
        ...
        (
            loc_context["filterkeys"],
            loc_context["limitkeys"],
            loc_context["delimitkeys"],
            loc_context["amountkeys"],
            loc_context["specialkeys"],
        ) = uri_to_internal(keys)
        try:
    ...
    if malojaconfig["DEV_MODE"]:
        jinja_environment.cache.clear()
...
def redirect_track(artists, title):
    redirect(
        ...
        + title
        + "&"
        + "&".join("artist=" + artist for artist in artists.split("/"))
    )
