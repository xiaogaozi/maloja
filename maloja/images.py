...
engine = sql.create_engine(
    f"sqlite:///{data_dir['cache']('imagecache.sqlite')}", echo=False
)
meta = sql.MetaData()
...
def get_image_from_cache(track_id=None, artist_id=None, album_id=None):
    ...
    entity_id, table = get_id_and_table(
    ...
    with dblock:
        with engine.begin() as conn:
            op = (
                ...
                .returning(DB[table].c.id, DB[table].c.localproxyurl)
            )
            result = conn.execute(op).all()

        for row in result:
            try:
                targetpath = data_dir["cache"](
                    "images", row.localproxyurl.split("/")[-1]
                )
...
resolver = ThreadPoolExecutor(
    max_workers=MAX_RESOLVE_THREADS, thread_name_prefix="image_resolve"
)
...
image_resolve_controller = {"artists": set(), "albums": set(), "tracks": set()}
...
def resolve_image(artist_id=None, track_id=None, album_id=None):
    result = get_image_from_cache(
        artist_id=artist_id, track_id=track_id, album_id=album_id
    )
    ...
    try:
        ...
        if artist_id:
            ...
            result = thirdparty.get_image_track_all(
                (entity["artists"], entity["title"])
            )
        elif album_id:
            result = thirdparty.get_image_album_all(
                (entity["artists"], entity["albumtitle"])
            )
        ...
        if result is not None:
            ...
            if result["value"] is None:
                ...
                if malojaconfig["FANCY_PLACEHOLDER_ART"]:
                    ...
                    if artist_id:
                        result["value"] = (
                            placeholder_url + f"tiles&colors={artist_id % 100}"
                        )
                    if track_id:
                        result["value"] = (
                            placeholder_url + f"triangles&colors={track_id % 100}"
                        )
                    if album_id:
                        result["value"] = (
                            placeholder_url + f"joy-division&colors={album_id % 100}"
                        )
                else:
                    if artist_id:
...
def get_all_possible_filenames(artist=None, track=None, album=None):
    if track:
        ...
        title, artists = (
            clean(album["albumtitle"]),
            [clean(a) for a in album.get("artists") or []],
        )
        superfolder = "albums/"
    elif artist:
        artist = clean(artist)
    ...
    if track or album:
        ...
        if len(filenames) == 0:
            filenames.append(str(hash((frozenset(artists), title))))
    else:
        ...
        filenames = list(set(filenames))
        if len(filenames) == 0:
            filenames.append(str(hash(artist)))
...
def set_image(b64, **keys):
    ...
    set_image_in_cache(
        **idkeys, url=os.path.join("/images", folder, filename), local=True
    )
