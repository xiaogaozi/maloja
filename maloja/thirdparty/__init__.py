...
services = {"proxyscrobble": [], "import": [], "metadata": []}
...
def get_image_track_all(track):
    with thirdpartylock:
        for service in services["metadata"]:
            if "track" not in service.metadata["enabled_entity_types"]:
                continue
            try:
...
def get_image_artist_all(artist):
    with thirdpartylock:
        for service in services["metadata"]:
            if "artist" not in service.metadata["enabled_entity_types"]:
                continue
            try:
...
def get_image_album_all(album):
    with thirdpartylock:
        for service in services["metadata"]:
            if "album" not in service.metadata["enabled_entity_types"]:
                continue
            try:
...
class ProxyScrobbleInterface(GenericInterface, abstract=True):
    proxyscrobble = {"required_settings": [], "activated_setting": None}
    ...
    def active_proxyscrobble(self):
        return (
            all(
                self.settings[key] not in [None, "ASK", False]
                for key in self.proxyscrobble["required_settings"]
            )
            and malojaconfig[self.proxyscrobble["activated_setting"]]
        )
...
class ImportInterface(GenericInterface, abstract=True):
    scrobbleimport = {"required_settings": [], "activated_setting": None}
    ...
    def active_import(self):
        return (
            all(
                self.settings[key] not in [None, "ASK", False]
                for key in self.scrobbleimport["required_settings"]
            )
...
class MetadataInterface(GenericInterface, abstract=True):
    metadata = {"required_settings": [], "activated_setting": None}
    ...
    def active_metadata(self):
        return (
            ...
            and self.identifier in malojaconfig["METADATA_PROVIDERS"]
        )
    ...
    def get_image_artist(self, artist):
        ...
        url = self.metadata["artisturl"].format(artist=artiststring, **self.settings)
        log(f'Get artist image of "{artist}" from {url}')
    ...
    def get_image_album(self, album):
        ...
        artiststring = urllib.parse.quote(", ".join(artists or []))
        titlestring = urllib.parse.quote(title)
        url = self.metadata["albumurl"].format(
            artist=artiststring, title=titlestring, **self.settings
        )
        log(f'Get album image of "{title}" from {url}')
        response = requests.get(url, headers={"User-Agent": self.useragent})
    ...
    def _parse_response(self, resp, data):
        ...
        for node in self.metadata[resp]:
            try:
                ...
                handleresult = self.handle_json_result_error(
                    data
                )  # allow the handler to throw custom exceptions
...
__all__ = ["lastfm", "spotify", "musicbrainz", "audiodb", "deezer", "maloja"]
from . import *
