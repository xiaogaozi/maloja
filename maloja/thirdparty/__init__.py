...
class MetadataInterface(GenericInterface,abstract=True):
    ...
    def get_image_album(self,album):
        ...
        titlestring = urllib.parse.quote(title)
        url = self.metadata["albumurl"].format(artist=artiststring,title=titlestring,**self.settings)
        ...
        if self.metadata["response_type"] == "json":
            ...
            if "response_parse_album_items" in self.metadata:
                ...
                for item in self._parse_response("response_parse_album_items", data):
                    artist_name = self._parse_response("response_parse_album_item_artist_name", item)
                    if artist_name in artists:
                        imgurl = self._parse_response("response_parse_album_item_imgurl", item)
                        break
            else:
